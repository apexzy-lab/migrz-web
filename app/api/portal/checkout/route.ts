import { audit, json, paymentAmount, paypalAccessToken, paypalBaseUrl, plans, portalEnv, providerForCountry, randomId, requireSession, validPlan } from "@/app/portal/server";
...
export async function POST(request: Request) {
  try {
    ...
    const plan = body.plan; const provider = providerForCountry(session.user.countryResidence); const price = paymentAmount(provider, plan); const paymentId = randomId("pay_"); const now = Date.now(); const origin = new URL(request.url).origin;
    ...
    await portalEnv.DB.prepare("UPDATE payments SET provider_reference=?,checkout_url=?,updated_at=? WHERE id=?").bind(reference, checkoutUrl, Date.now(), paymentId).run(); await audit("checkout_created", "payment", paymentId, session.user.id, { provider, plan, currency: price.currency, amountMinor: price.amountMinor });
