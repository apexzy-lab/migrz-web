...
export async function GET(request: Request) {
  ...
  const result = await portalEnv.DB.prepare("SELECT id,file_name AS fileName,content_type AS contentType,size,status,created_at AS createdAt FROM documents WHERE user_id=? AND status!='deleted' ORDER BY created_at DESC").bind(session.user.id).all(); return json({ documents: result.results });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const documentId = randomId("doc_"); const extension = file.type === "application/pdf" ? "pdf" : file.type === "image/png" ? "png" : "jpg"; const key = `${session.user.id}/${application.id}/${documentId}.${extension}`;
    await portalEnv.DOCUMENTS.put(key, file.stream(), { httpMetadata: { contentType: file.type }, customMetadata: { owner: session.user.id, documentId } });
    await portalEnv.DB.prepare("INSERT INTO documents (id,user_id,application_id,r2_key,file_name,content_type,size,status,created_at) VALUES (?,?,?,?,?,?,?,?,?)").bind(documentId, session.user.id, application.id, key, file.name.slice(0, 240), file.type, file.size, "stored", Date.now()).run(); await audit("document_uploaded", "document", documentId, session.user.id, { size: file.size, contentType: file.type }); return json({ document: { id: documentId, fileName: file.name, contentType: file.type, size: file.size, status: "stored" } }, 201);
