...
export async function POST(request: Request) {
  ...
  else await portalEnv.DB.prepare("INSERT INTO appointment_requests (id,public_id,application_id,user_id,requested_start,duration_minutes,timezone,applicant_note,status,confirmed_start,meeting_url,provider,provider_event_uri,provider_invitee_uri,cancel_url,reschedule_url,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(appointmentId, publicId, application.id, session.user.id, requestedStart, durationMinutes, timezone, note, status, confirmedStart, booking?.meetingUrl || null, booking ? "calendly" : "manual", booking?.eventUri || null, booking?.inviteeUri || null, booking?.cancelUrl || null, booking?.rescheduleUrl || null, now, now).run();
