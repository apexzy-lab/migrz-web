...
export async function POST(request: Request) {
  ...
  const now = Date.now(); await portalEnv.DB.prepare("UPDATE applications SET status='submitted',review_status='submitted',current_section=4,submitted_at=?,updated_at=? WHERE id=?").bind(now, now, application.id).run(); await audit("application_submitted", "application", application.id, session.user.id); const submitted = await portalEnv.DB.prepare("SELECT public_id AS publicId FROM applications WHERE id=?").bind(application.id).first<{ publicId: string }>(); return json({ ok: true, submittedAt: now, publicId: submitted?.publicId });
}
