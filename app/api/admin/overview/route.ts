...
export async function GET(request: Request) {
  ...
  const [metrics, applications, payments, activity, admins, appointments] = await Promise.all([
    ...
    portalEnv.DB.prepare(`SELECT ar.id,ar.public_id AS publicId,ar.status,ar.requested_start AS requestedStart,ar.duration_minutes AS durationMinutes,ar.timezone,ar.applicant_note AS applicantNote,ar.confirmed_start AS confirmedStart,ar.meeting_url AS meetingUrl,ar.admin_note AS adminNote,ar.created_at AS createdAt,ar.updated_at AS updatedAt,a.public_id AS applicationPublicId,u.email FROM appointment_requests ar JOIN applications a ON a.id=ar.application_id JOIN users u ON u.id=ar.user_id ORDER BY CASE ar.status WHEN 'requested' THEN 1 WHEN 'confirmed' THEN 2 ELSE 3 END,coalesce(ar.confirmed_start,ar.requested_start) ASC LIMIT 200`).all(),
  ]);
