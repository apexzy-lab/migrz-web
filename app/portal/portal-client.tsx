...
type PortalSession = { loaded: boolean; authenticated: boolean; user: null | { email: string; countryResidence: string; preferredPlan: "standard" | "accelerated"; paid: boolean; paidPlan: "standard" | "accelerated" | null }; integrations: { email: boolean; paystack: boolean; paypal: boolean; wise: boolean } };
...
function Assessment() {
  const [section, setSection] = useState(1); const [answers, setAnswers] = useState<AssessmentAnswers>(emptyAnswers); const [message, setMessage] = useState("Loading your saved assessment…"); const [busy, setBusy] = useState(false); const [applicationStatus, setApplicationStatus] = useState<"loading" | "draft" | "submitted">("loading"); const [submittedAt, setSubmittedAt] = useState<number | null>(null); const sectionData = [{ title: "About you", copy: "Confirm the personal details and destination that help us understand your circumstances and timing." }, { title: "Career & evidence", copy: "Tell us what you have done, how your work mattered and what can verify it." }, { title: "Review your answers", copy: "Check your information before submitting it to the Migrz assessment team." }, { title: "Submit assessment", copy: "Submit your completed record for professional review." }];
  useEffect(() => { void (async () => { const response = await fetch("/api/portal/application", { cache: "no-store" }); const data = await response.json() as { application?: { currentSection?: number; answers?: Partial<AssessmentAnswers>; status?: string; submittedAt?: number | null } }; if (response.ok && data.application) { setSection(Math.min(4, data.application.currentSection || 1)); setAnswers({ ...emptyAnswers, ...data.application.answers }); setApplicationStatus(data.application.status === "submitted" ? "submitted" : "draft"); setSubmittedAt(data.application.submittedAt || null); setMessage(data.application.status === "submitted" ? "Assessment submitted." : "All changes are stored securely."); } else { setApplicationStatus("draft"); setMessage("Start with the questions below."); } })(); }, []);
