...
type View = "home" | "assessment" | "documents" | "payment" | "appointment" | "help";
  ...
  return <div className="portal-shell">
    ...
    <div className="portal-app">
      ...
      <main className="portal-main" id="main">
        ...
        {view === "appointment" && <Appointment reviewComplete={reviewComplete} applicationPublicId={applicationState.application?.publicId || ""} />}
        {view === "help" && <Help />}
      </main>
    </div>
...
type PortalAppointment = { id: string; publicId: string; status: string; requestedStart: number; durationMinutes: number; timezone: string; applicantNote: string; confirmedStart: number | null; meetingUrl: string | null; adminNote: string };
function Appointment({ reviewComplete, applicationPublicId }: { reviewComplete: boolean; applicationPublicId: string }) {
  const [appointment, setAppointment] = useState<PortalAppointment | null>(null); const [requestedStart, setRequestedStart] = useState(""); const [duration, setDuration] = useState(60); const [note, setNote] = useState(""); const [timezone] = useState(() => Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC"); const [message, setMessage] = useState("Loading appointment status…"); const [busy, setBusy] = useState(false);
  ...
  const requestAppointment = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setMessage("Saving your call request…"); const response = await fetch("/api/portal/appointment", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestedStart: new Date(requestedStart).getTime(), durationMinutes: duration, timezone, note }) }); const data = await response.json() as { appointment?: PortalAppointment; error?: string }; setBusy(false); if (!response.ok) return setMessage(data.error || "Unable to request the call."); setAppointment(data.appointment || null); setMessage("Your call request has been sent to Migrz."); };
  ...
  const callTime = appointment?.status === "confirmed" && appointment.confirmedStart ? appointment.confirmedStart : appointment?.requestedStart;
  return <div className="portal-subpage portal-appointment"><header><span className="portal-eyebrow">Review complete · {applicationPublicId}</span><h1>Book your assessment call.</h1><p>Request a suitable time to discuss your written assessment, questions and recommended next steps with Migrz. Calls are capped at 60 minutes.</p></header>{appointment ? <section className={`portal-appointment-status is-${appointment.status}`}><span>{appointment.status === "confirmed" ? "Confirmed appointment" : appointment.status === "requested" ? "Confirmation pending" : appointment.status}</span><h2>{callTime ? new Date(callTime).toLocaleString([], { dateStyle: "full", timeStyle: "short" }) : "Time being arranged"}</h2><p>{appointment.durationMinutes} minutes · {appointment.timezone}</p><dl><div><dt>Call reference</dt><dd>{appointment.publicId}</dd></div><div><dt>Status</dt><dd>{appointment.status.replaceAll("_", " ")}</dd></div></dl>{appointment.meetingUrl && <a className="portal-primary" href={appointment.meetingUrl} target="_blank" rel="noreferrer">Open meeting link <Arrow /></a>}{appointment.adminNote && <small>{appointment.adminNote}</small>}{appointment.status === "requested" && <button className="portal-link-button" onClick={() => setAppointment(null)}>Change requested time</button>}</section> : <form className="portal-appointment-form" onSubmit={requestAppointment}><div><span className="portal-eyebrow">Request a time</span><h2>Choose your preferred call window.</h2><p>Migrz will confirm the final appointment by email and in this portal.</p></div><label>Preferred date and time<input type="datetime-local" value={requestedStart} onChange={(event) => setRequestedStart(event.target.value)} required /></label><label>Call length<select value={duration} onChange={(event) => setDuration(Number(event.target.value))}><option value={30}>30 minutes</option><option value={45}>45 minutes</option><option value={60}>60 minutes</option></select><small>Calls cannot exceed one hour.</small></label><label className="portal-wide-field">What would you like to cover?<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional questions or topics for the Migrz team" maxLength={1000} /></label><div className="portal-appointment-zone"><span>Your timezone</span><strong>{timezone}</strong></div><footer><span role="status">{message}</span><button className="portal-primary" type="submit" disabled={busy}>{busy ? "Sending request…" : "Request this call"} <Arrow /></button></footer></form>}</div>;
}
